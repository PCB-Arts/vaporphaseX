...
// redefines delay function of communication stack
...
static void MX_I2C1_Init(void) {
  ...
  HAL_I2C_DeInit(&hi2c1); /* FIXME wat? */
}
...
static void MX_FMC_Init(void) {
  ...
  BSP_SDRAM_DeInit();
  if (BSP_SDRAM_Init() != SDRAM_OK) { /* FIXME wat? */
    Error_Handler();
  }
}
...
static void MX_GPIO_Init(void) {
  ...
  /*Configure GPIO Pin : SELFTEST */
  GPIO_InitStruct.Pin = SELFTEST_Pin;
 ...
 * @retval None
 */
void assert_failed(uint8_t *file, uint32_t line) { asm volatile("bkpt"); }
