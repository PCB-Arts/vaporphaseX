...
[#if data.parameters.tgfx_datareader_block_size != "0"]
    ...
    uint8_t buffer1[${data.parameters.tgfx_datareader_block_size}]; // 32 bit allign
    uint8_t buffer2[${data.parameters.tgfx_datareader_block_size}]; // 32 bit allign
    bool readToBuffer1;
[/#if]
    const uint32_t flashEndAddress;
};
